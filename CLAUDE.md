...
- `study.js` + `study-lessons.js` + `lib/study.js` + `lib/questions.js` — the **Grammar Gym** SRS trainer (`#/study`, hidden route): `lib/study.js` is the pure FSRS-lite engine (per-point D/S, stage ladder, mastery gate, queue builder, session helpers, `lessonOrder`/`unitProgress`/`testOutResult` — all `now`-parametrised, unit-tested; store shape carries an internal `v` + `UPGRADERS` chain, currently **v2** = adds `settings.placed[]`/`examLevel`); `lib/questions.js` is the pure typed-cloze generator + answer arbitration (blank `p` tokens, accept surface OR kana, NFKC/kana-fold + Levenshtein-1 close-match); `study.js` is the shell — the **course home** (five level cards + progress rings → unit accordion from `data/grammar-units.json`, ONE priority-ordered ▶ Continue button, exam-priority lever) plus the untouched R2 session runner (per-answer write-through to `jwh-study-v1`, mid-session resume); `study-lessons.js` is the lazy-imported 3-beat lessons + placement sweep + test-out flows. `data/grammar-units.json` groups all 353 points into named thematic units (validated by `validateUnits` in `scripts/validate-grammar.mjs`: every point in exactly one unit, sizes 6–16). `.stok` tokens NEVER `.jp`; scramble/MCQ/gate/checkpoints land in later rounds. Plan: `specs/plans/2026-07-17-grammar-mastery-program.md`.
