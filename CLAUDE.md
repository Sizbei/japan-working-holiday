...
- `study.js` + `lib/study.js` + `lib/questions.js` — the **Grammar Gym** SRS trainer (`#/study`, hidden route): `lib/study.js` is the pure FSRS-lite engine (per-point D/S, stage ladder, mastery gate, queue builder, session helpers — all `now`-parametrised, unit-tested); `lib/questions.js` is the pure typed-cloze generator + answer arbitration (blank `p` tokens, accept surface OR kana, NFKC/kana-fold + Levenshtein-1 close-match); `study.js` is the session-runner UI (typed cloze, per-answer write-through to `jwh-study-v1`, mid-session resume — `.stok` tokens NEVER `.jp`; scramble/MCQ/lessons/gate land in later rounds). Plan: `specs/plans/2026-07-17-grammar-mastery-program.md`.
