...
- `grammar.js` + `lib/grammar.js` — JLPT grammar reference (`#/grammar`, hidden route linked from Survival Japanese): 352 points across `data/grammar-n[1-5].json` (per-level lazy fetch, SW stale-while-revalidate), token-level furigana/hover (own `.gtok` selector — NEVER `.jp`, which lang.js claims globally; cross-dismiss via `jwh:popover-open {owner}`), ✓/◆ tracking (`jwh-grammar-v1`). Data PRs run `scripts/validate-grammar.mjs` + a reading-accuracy critic; jukujikun readings are FUSED f segments.
