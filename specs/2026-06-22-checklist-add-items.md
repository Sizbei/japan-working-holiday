...
- **`renderChecklist`** — for each baked phase `p`, the rendered list = `orderItems([...p.items, ...customFor(p.phase)], order[pi])` (custom items marked `_custom:true`). After the baked phases, render a synthetic **"My tasks"** accordion group (collapse id `chk-phase-mine`, the `<ul>` carries **`data-phase="mine"`** so its reorder key is the string `"mine"` — distinct from the numeric baked indices) containing custom items whose `phase` is exactly `"My tasks"` (or whose `phase` matches no baked phase — orphan-safe). Show the group only if it has items. Verified safe against existing code: `checkItemHTML` already `esc()`s the task (`content.js` `${esc(it.task)}`); custom items have no `requires` so the lock logic and `knownIds` are unaffected; `calendar.js` does not import `content.js` (no cycle — the `lib/checklist.js` store wrapper is the boundary).
