...
1. **Grid (#/people)** — kanji accent `縁` (en — "connection/fate"), serif `People` heading, italic lede with the privacy promise in bold. Toolbar: search (`names, places, notes…`), sort (`recently met ▾` / name / recently seen), crimson `＋ Add person`. Tag-filter chip row (All + user's tags). Cards: initials avatar (deterministic per-person hue), name + reading, `from · languages` mono subline, **"Met Jul 4 · Fuji Rock — camped in the next tent"** (the emotional hook), tag chips, italic notes preview with left rule, footer = last-seen (green dot ≤7 days, amber "☾ N days ago" when drifting) + contact link.
2. **Detail panel** — same anchored-popover pattern as the calendar's event panel (grid dims, Esc closes, focus returns). Label/value rows: met (when · where · context), from, speaks, birthday, contact, last seen. Free-form notes block with "updated <date>". Actions: **✓ Seen today** (primary — the one-tap habit), ✎ Edit, ＋ Note, Delete (confirm).
