...
- [ ] View toggle ⊞ Cards / ☰ List in the toolbar (persisted raw key `jwh-people-view-v1`); `listHTML()` renders the dense row table (row click → drawer).
- [ ] `docs/assets/people.js` `mountPeople()`: render grid (`minmax(280px,1fr)`) or list from store through search/sort/filter state incl. the leading `★ starred` chip (module-local, not persisted); empty state (⛩️👋 + CTA) when no people AND no active filter; "no results" variant when filtered (distinct copy per design checklist).
...
- [ ] **Detail drawer** (not the calendar popover): fixed right-edge surface (380px; full-width sheet ≤560px), scrim behind, slide-in 240ms ease-out (reduce-motion gated), Esc/✕/scrim closes, focus returns to opener. Rows per v2 mock (met · nationality/from · lives · address-as · next-plan · leaves · met-through · food · speaks · birthday · seen ×N · contact); actions `✓ Seen today` (lastSeen=today AND seenCount++ → data-changed), `★` toggle, `✎ Edit`, `＋ Note` (appends to notes + notesUpdated), `Delete` (confirmModal).
